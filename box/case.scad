...
btn      = 12;       // корпус 12×12
btn_can  = 3.3;      // высота корпуса за лицом (без актуатора)
act_d    = 6.55;     // диаметр актуатора
act_out  = 8.25;     // насколько актуатор торчит над корпусом
btn_legs = 3.5;      // ноги за корпусом

/* ---------- SCD41 модуль (замер) ---------- */
scd_l = 21.6; scd_w = 13.4; scd_t = 1.6;   // плата
can   = 8; can_h = 6.5;                      // металлический сенсор (мембрана сверху)
scd_can_off = 2;                             // отступ сенсора от дальнего (правого) края
scd_hdr_n = 4;                               // линейка 1×4 на левом коротком крае
...
dup_h   = 17;    // длина дюпон-мама (худший случай)
...
// SCD: низ-право; пины 1×4 на левом крае, сенсор смещён к правому краю → в отверстие лица
scd_x = wall + inner_w - scd_l - fit;
scd_y = wall + fit;
can_cx = scd_x + scd_l - scd_can_off - can/2;   // сенсор у правого края платы
can_cy = scd_y + scd_w/2;                        // по центру ширины

// Батарея: по центру (мала относительно корпуса → держат рёбра полки)
...
module ghosts(show_bat = true, show_shelf = true) {
    ...
    // Батарея (выключатель отдельный на проводе — не на боксе)
    if (show_bat)
...
// Паз 7×3 под отдельный выключатель (на проводе) — в левой стенке у батарейного
// отсека, доступен при открытой крышке.
module switch_slot() {
    translate([-1, wall + inner_h/2 - sw_slot_w/2, bat_z + bat_h/2 - sw_slot_h/2])
