...
пользователь спрашивает про прошивку, отладку на железе, диагностику,
сборку «от деталей на столе», что такое I2C/SPI/GPIO, как паять,
как подключать провода — **открой `BRINGUP.md`** в корне проекта.
Это пошаговая инструкция для пользователя БЕЗ опыта в электронике:
описание деталей, инструменты, базовые понятия, схемы проводки для
каждого этапа, ожидаемый Serial-вывод, дерево диагностики и параметры
для подкрутки. Не отсылай к внешним ресурсам — там уже всё есть.
...
> **АКТУАЛЬНО:** реализован минималистичный **CO2-only** UI (см. `draw_main` /
> `draw_graph` в `src/main.cpp`). Со всех сторон оставлены поля
> (`PAD_L/PAD_R/PAD_T/PAD_B`), чтобы у рамки экрана ничего не срезалось.
> MAIN — крупное число CO2 по центру + светофор + слово-статус. Графики —
> одна чистая линия, растянутая на всю ширину (старый баг «40% слева пусто»
> и «24ч показывал в разы больше» исправлены: окно графика теперь считается
> из реального `MEASUREMENT_INTERVAL`, а точки интерполируются на всю ширину).
> Разделы ниже про T/RH, светофоры по трём показателям и тройной layout —
> исторические (двухкнопочная 9-экранная версия).
...
## Экраны и навигация (CO2-only, ОДНА кнопка)

> **АКТУАЛЬНАЯ реализация (на железе).** Корпус собран с **одной** кнопкой
> (GPIO5), и пользователь смотрит **только CO2**. Поэтому UI переделан:
> 4 экрана, одна кнопка листает их по кругу. Прежний девятиэкранный
> двухкнопочный вариант (CO2/T/RH × 1h/24h/7d) — исторический, не используется.
> Температура/влажность по-прежнему **измеряются и пишутся в файлы**, но на
> экран не выводятся.

```
Index | Экран
------|----------------------------------
0     | MAIN — крупное текущее CO2 + статус (GOOD/FAIR/POOR/VENTILATE)
1     | График CO2 за 1 час
2     | График CO2 за 24 часа
3     | График CO2 за 7 дней
```

### Навигация — pure функция в `lib/logic/`

```cpp
// logic.h
int cycle_screen(int current_screen, int n_screens);  // 0→1→2→3→0
```

- **Короткое нажатие кнопки (GPIO5)** → `cycle_screen` → следующий экран по кругу.
- **Пробуждение по таймеру (раз в 15 мин)** → новое измерение и **принудительный
  возврат на MAIN** (`current_screen = SCREEN_MAIN`), чтобы по таймеру всегда
  показывалось свежее значение CO2.

`RTC_DATA_ATTR int current_screen` переживает deep sleep. Прежние
`cycle_param` / `cycle_period` оставлены в logic для совместимости/тестов.
...
// ВАЖНО (исправленный баг): SCD41 хранит temperature offset, который ВЫЧИТАЕТСЯ
// из показания. Заводское значение = 4.0 °C и рассчитано на НЕПРЕРЫВНЫЙ режим
// (саморазогрев ~4°). Мы меряем в single-shot — саморазогрева почти нет, поэтому
// заводские 4° давали стабильное занижение на ~4–6° («то -4, то -6»). Для
// single-shot офсет должен быть ~0. Прошивка задаёт его КАЖДЫЙ цикл перед замером
// (powerDown сбрасывает RAM-настройку датчика). После сборки сверить с отдельным
// термометром и подкрутить: показывает на X° меньше реального → УМЕНЬШИ офсет на X.
