...
// ПРОВЕРЕНО 2026: 10 парных замеров с Xiaomi — расхождение в среднем +0.1 °C
// (максимум 0.6), т.е. температура калибровки не требует, офсет остаётся 0.
...
// Подкрутка: прибор показывает на X% МЕНЬШЕ реального → УВЕЛИЧЬ офсет на X.
//
// ЗАМЕР 2026: 10 парных сравнений (36–54 %RH, 22.9–26.8 °C) с гигрометром
// Xiaomi (SHT3x-класс, паспорт ±3 %RH) и с ноунейм-китайцем:
//   Xiaomi − наш: 10,11,10,9,10,10,10,8,9,8  → среднее +9.5, медиана +10
//   ноунейм − наш: 1,2,1,0,3,1,0,0,0,0       → среднее +0.8 (совпадает с нашим)
// Линейная регрессия наш→Xiaomi даёт y = 0.87·x + 14.9, т.е. почти чистый
// сдвиг: константа +9.5 держит ошибку в пределах ±1.5 %RH во всём диапазоне,
// поэтому усложнять (gain+offset) смысла нет.
// Почему верим Xiaomi, а не «двум против одного»: (1) у SCD41 RH — вторичный
// канал с паспортной точностью ±6 %RH, у Xiaomi ±3 %; (2) ноунейм при этом
// завышает температуру на +0.5 °C, т.е. сам плохо откалиброван; (3) физика:
// 37 % при 25 °C в июле — это влажность отопительного сезона, для лета
// нереально сухо, а 47–50 % (и 62 % в конце августа) — норма.
// САМОРАЗОГРЕВА в занижении не винить: температура нашего датчика совпадает
// с Xiaomi (+0.1 °C в среднем), т.е. кристалл не горячее воздуха.
#define HUMIDITY_OFFSET         9.5f
